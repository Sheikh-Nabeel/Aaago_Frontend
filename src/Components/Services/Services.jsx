  ...
  return (
    <>
      <div className=" h-[500px] rounded-3xl my-7 w-[95%] mx-auto bg-black text-white">
        <div
          className="relative bg-cover bg-center h-[500px] rounded-3xl overflow-hidden flex items-center justify-center"
        ...
        >
          ...
          <div className="relative z-10 text-center px-4">
            ...
            <h1 className="text-4xl md:text-5xl font-bold leading-tight">
              Experience Convenience Our Service <br className='max-lg:hidden' /> Offerings
            </h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 relative mb-32 max-lg:mb-0 py-12 flex flex-col-reverse md:flex-row items-center gap-10">
        ...
        <div className="w-[50%] max-lg:w-[100%] ">
          ...
          <ul className="space-y-3 mb-6 text-gray-700">
            <li className="flex items-start gap-2"><FaCheckCircle className="text-[#0B996F] mt-1" />A global ride-hailing network that empowers drivers & customers.</li>
            <li className="flex items-start gap-2"><FaCheckCircle className="text-[#0B996F] mt-1" />Creating financial freedom and opportunities.</li>
            <li className="flex items-start gap-2"><FaCheckCircle className="text-[#0B996F] mt-1" />Sustainable & innovative mobility solutions.</li>
          </ul>
          <p className="mb-6"><span className="font-bold">Our vision is simple</span> – to build a ride-hailing platform where drivers earn fairly, customers enjoy reliable services, and communities thrive through innovation.</p>
          <div className="flex items-start gap-3  p-2 rounded-lg ">
            <div className="bg-green-200 p-3  rounded-full"><FaShieldAlt className="text-[#0B996F] text-2xl " /></div>
            <div>
              <p className="font-bold text-[#0B996F] text-lg">Safety Measures</p>
              <p className="text-gray-600 text-sm">Verified drivers, real-time tracking & No Male Companion rides for ladies and families – your peace of mind comes first.</p>
            </div>
          </div>
        </div>
      ...
      <div className="text-center mb-12">
        <button className="bg-[#1E784B] text-white text-sm font-semibold px-8 py-3 rounded-full mb-4">The Perfect Ride Awaits</button>
        <h2 className="text-5xl font-bold max-lg:text-4xl text-[#0B996F]">One App, Unlimited Possibilities</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-6 px-20 max-lg:px-4 bg-white">
        {services.map((service, index) => (
          <div key={index} className="bg-[#E2F4EA] rounded-lg shadow px-5 py-3 flex flex-col justify-between">
            <img src={service.image} alt={service.title} className="h-32 w-full rounded-2xl object-cover" />
            <h3 className="text-xl font-semibold mt-4"><span className="mr-2 text-sm">{service.icon}</span>{service.title}</h3>
            <p className="text-xs text-gray-700 mt-2 flex-grow">{service.description}</p>
            <button onClick={() => setSelectedService(service)} className="bg-[#319A64] text-white px-2 py-1 mt-4 rounded hover:bg-green-700 w-fit">View Details</button>
          </div>
        ))}
      </div>
      ...
      {selectedService && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center px-4">
          <div className="flex flex-col lg:flex-row gap-6 lg:gap-10 rounded-xl bg-white p-6 max-w-5xl w-full relative overflow-y-auto max-h-[90vh]">
            <button
            ...
            <div className="w-full lg:w-[60%] overflow-y-auto">
              ...
              <p className="text-sm text-gray-700 mb-4">
                ...
                <>
                  ...
                  <ul className="text-sm text-gray-700 list-disc ml-5 space-y-3">
                    <li className='list-none'><span className="font-bold">{selectedService.span1}</span> {selectedService.p1}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span2}</span> {selectedService.p2}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span3}</span> {selectedService.p3}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span4}</span> {selectedService.p4}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span5}</span> {selectedService.p5}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span6}</span> {selectedService.p6}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span7}</span> {selectedService.p7}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span8}</span> {selectedService.p8}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span9}</span> {selectedService.p9}</li>
                    <li className='list-none'><span className="font-bold">{selectedService.span10}</span> {selectedService.p10}</li>
                  </ul>
                </>
      ...
      <div ref={ref} className="bg-[#e8f9f1] text-center py-16 px-4 mb-12 max-lg:py-8">
        <h2 className="text-5xl font-bold max-lg:text-4xl text-[#0B996F] mb-12">Breaking Barriers, Building a Fairer Future<br />for One Billion People</h2>
        <div className="flex flex-col md:flex-row justify-center items-center gap-36 max-lg:gap-10">
          <div><p className="text-5xl font-bold text-[#555D55]">{inView && <CountUp end={5} duration={2} />}</p><p className="text-xl text-[#555D55] mt-2">countries</p></div>
          <div><p className="text-5xl font-bold text-[#555D55]">{inView && <CountUp end={144} duration={2} />}</p><p className="text-xl text-[#555D55] mt-2">cities</p></div>
          <div><p className="text-5xl font-bold text-[#555D55]">{inView && <CountUp end={1500} duration={2.5} separator="," />}</p><p className="text-xl text-[#555D55] mt-2">Registered Members</p></div>
        </div>
      </div>
    </>
  );
